...
class TestTypeOfStatisticalProcessing(tests.IrisGribTest):
    ...
    def test_sum(self, mock_set):
        ...
        mock_set.assert_any_call(mock.sentinel.grib, "typeOfStatisticalProcessing", 1)
    ...
    def test_unrecognised(self, mock_set):
        ...
        _product_definition_template_8_10_and_11(cube, mock.sentinel.grib)
        mock_set.assert_any_call(mock.sentinel.grib, "typeOfStatisticalProcessing", 255)

    @mock.patch.object(eccodes, "codes_set")
    def test_multiple_cell_method_coords(self, mock_set):
        cube = self.cube
        cell_method = CellMethod(method="sum", coords=["time", "forecast_period"])
        cube.add_cell_method(cell_method)
...
class TestTimeCoordPrerequisites(tests.IrisGribTest):
    ...
    def test_multiple_points(self, mock_set):
        ...
        with self.assertRaisesRegex(ValueError, "Expected length one time coordinate"):
...
class TestEndOfOverallTimeInterval(tests.IrisGribTest):
    ...
    def test_default_calendar(self, mock_set):
        ...
        coord = DimCoord(
            23.0, "time", bounds=[0.452, 20314.452], units=Unit("hours since epoch")
        )
        ...
        mock_set.assert_any_call(grib, "monthOfEndOfOverallTimeInterval", 4)
        mock_set.assert_any_call(grib, "dayOfEndOfOverallTimeInterval", 26)
...
class TestNumberOfTimeRange(tests.IrisGribTest):
    ...
    def test_other_cell_methods(self, mock_set):
        ...
        coord = DimCoord(23, "time", bounds=[0, 24], units=Unit("hours since epoch"))
        cube.add_aux_coord(coord)
