...
class Test(tests.IrisGribTest):
    def setUp(self):
        self.coord = DimCoord(
            0, "time", units=Unit("hours since epoch", calendar="standard")
        )
    ...
    def test_hours(self, mock_set):
        ...
        set_time_range(self.coord, mock.sentinel.grib)
        mock_set.assert_any_call(mock.sentinel.grib, "indicatorOfUnitForTimeRange", 1)
        mock_set.assert_any_call(mock.sentinel.grib, "lengthOfTimeRange", upper - lower)
    ...
    def test_days(self, mock_set):
        ...
        mock_set.assert_any_call(
            mock.sentinel.grib, "lengthOfTimeRange", (upper - lower) * 24
        )
    ...
    def test_fractional_hours(self, mock_set_long):
        ...
        msg = (
            r"Truncating floating point lengthOfTimeRange 10\.8?9+ "
            "to integer value 10"
        )
        self.assertRegex(str(warn[0].message), msg)
        mock_set_long.assert_any_call(
            mock.sentinel.grib, "indicatorOfUnitForTimeRange", 1
        )
        mock_set_long.assert_any_call(
            mock.sentinel.grib, "lengthOfTimeRange", int(upper - lower)
        )
