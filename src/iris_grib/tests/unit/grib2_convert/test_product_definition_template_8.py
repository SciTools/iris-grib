...
"""
Tests for function
:func:`iris_grib._grib2_convert.product_definition_template_8`.
...
from iris_grib._grib2_convert import product_definition_template_8
...
class Test(tests.IrisGribTest):
    def setUp(self):
        ...
        self.patch_generating_process = self.patch(module + ".generating_process")
    ...
    def test_basic(self):
        product_definition_template_8(self.section, self.metadata, self.frt_coord)
        ...
        self.assertEqual(
            sorted(self.metadata.keys()), ["aux_coords_and_dims", "cell_methods"]
        )
        self.assertEqual(
            self.metadata["cell_methods"], [mock.sentinel.dummy_cell_method]
        )
