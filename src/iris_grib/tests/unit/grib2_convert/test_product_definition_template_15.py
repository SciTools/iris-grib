...
"""
Test function
:func:`iris_grib._grib2_convert.product_definition_template_15`.
...
from iris_grib.tests.unit.grib2_convert import LoadConvertTest
from iris_grib._grib2_convert import _MDI as MDI

from iris_grib._grib2_convert import product_definition_template_15
...
class Test(LoadConvertTest):
    def setUp(self):
        ...
        self.forecast_period_coord = DimCoord(6, "forecast_period", units="hours")
        self.forecast_ref_time_coord = DimCoord(
            18, "forecast_reference_time", units="hours since epoch"
        )
        self.height_coord = iris.coords.DimCoord(9999, long_name="height", units="m")

    def _translate(self, section):
        # Use pdt 4.15 to populate a metadata dict from the section 4 keys
        metadata = empty_metadata()
        product_definition_template_15(section, metadata, self.time_coord)
        return metadata
    ...
    def test_translation(self):
        ...
        expected["cell_methods"] = [CellMethod(coords=("area",), method="maximum")]
        expected["attributes"]["spatial_processing_type"] = "No interpolation"
