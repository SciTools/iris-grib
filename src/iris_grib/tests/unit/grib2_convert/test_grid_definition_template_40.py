...
"""
Unit tests for
:func:`iris_grib._grib2_convert.grid_definition_template_40`.
...
from iris_grib._grib2_convert import grid_definition_template_40
...
class Test_regular(tests.IrisGribTest):
    ...
    def expected(self, y_dim, x_dim, y_neg=True):
        ...
        y = iris.coords.DimCoord(
            y_points, standard_name="latitude", units="degrees_north", coord_system=cs
        )
        ...
        section = _Section(
            {
                ...
                "scaledValueOfEarthMinorAxis": MDI,
                "longitudes": np.array(
                    [0.0, 180.0, 0.0, 120.0, 240.0, 0.0, 120.0, 240.0, 0.0, 180.0]
                ),
                "latitudes": np.array(
                    [
                        -59.44440829,
