...
"""
Test function :func:`iris_grib._grib2_convert.product_definition_template_11`.
...
from iris_grib._grib2_convert import product_definition_template_11
...
class Test(tests.IrisGribTest):
    def setUp(self):
        ...
        module = "iris_grib._grib2_convert"
        ...
        self.patch_time_coord = self.patch(
            module + ".validity_time_coord", return_value=mock.sentinel.dummy_time_coord
        )
    ...
    def _check(self, request_warning):
        grib_lconv_opt = "iris_grib._grib2_convert.options"
        with mock.patch(grib_lconv_opt, warn_on_unsupported=request_warning):
            ...
            product_definition_template_11(section, metadata, forecast_reference_time)
            expected = {"cell_methods": [], "aux_coords_and_dims": []}
            expected["cell_methods"].append(CellMethod(method="sum", coords=("time",)))
            ...
            self.maxDiff = None
            self.assertEqual(
                metadata["aux_coords_and_dims"][-1], expected["aux_coords_and_dims"][0]
            )
            self.assertEqual(metadata["cell_methods"][-1], expected["cell_methods"][0])
