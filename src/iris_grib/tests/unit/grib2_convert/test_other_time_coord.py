...
"""
Test function :func:`iris_grib._grib2_convert.other_time_coord.
...
from iris_grib._grib2_convert import other_time_coord
...
class TestValid(tests.IrisGribTest):
    def test_t(self):
        ...
        expected = iris.coords.DimCoord(
            42, "forecast_reference_time", units="hours since epoch"
        )
        self.assertEqual(result, expected)

    def test_frt(self):
        rt = iris.coords.DimCoord(
            48, "forecast_reference_time", units="hours since epoch"
        )
        fp = iris.coords.DimCoord(6, "forecast_period", units="hours")
        result = other_time_coord(rt, fp)
...
class TestInvalid(tests.IrisGribTest):
    def test_t_with_bounds(self):
        rt = iris.coords.DimCoord(
            48, "time", units="hours since epoch", bounds=[36, 60]
        )
        fp = iris.coords.DimCoord(6, "forecast_period", units="hours")
    ...
    def test_frt_with_bounds(self):
        rt = iris.coords.DimCoord(
            48, "forecast_reference_time", units="hours since epoch", bounds=[42, 54]
        )
        fp = iris.coords.DimCoord(6, "forecast_period", units="hours")
        with self.assertRaisesRegex(ValueError, "bounds"):
            other_time_coord(rt, fp)
    ...
    def test_fp_with_bounds(self):
        rt = iris.coords.DimCoord(48, "time", units="hours since epoch")
        fp = iris.coords.DimCoord(6, "forecast_period", units="hours", bounds=[3, 9])
        with self.assertRaisesRegex(ValueError, "bounds"):
            other_time_coord(rt, fp)
    ...
    def test_vector_frt(self):
        rt = iris.coords.DimCoord(
            [0, 3], "forecast_reference_time", units="hours since epoch"
        )
        fp = iris.coords.DimCoord(6, "forecast_period", units="hours")
        with self.assertRaisesRegex(ValueError, "Vector"):
            other_time_coord(rt, fp)
