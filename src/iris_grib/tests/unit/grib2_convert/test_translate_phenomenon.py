...
"""
Tests for function :func:`iris_grib._grib2_convert.translate_phenomenon`.
...
from iris_grib._grib2_convert import Probability, translate_phenomenon
...
class Test_probability(tests.IrisGribTest):
    def setUp(self):
        ...
        target_module = "iris_grib._grib2_convert"
    ...
    def test_basic(self):
        translate_phenomenon(
            self.metadata, 7, 8, 9, None, None, None, probability=self.probability
        )
        # Check metadata.
        thresh_coord = DimCoord(
            [22.0], standard_name="air_temperature", long_name="", units="K"
        )
        self.assertEqual(
            self.metadata,
            {
    ...
    def test_no_phenomenon(self):
        ...
        expected_metadata["attributes"]["GRIB_PARAM"] = GRIBCode(2, 7, 77, 777)
